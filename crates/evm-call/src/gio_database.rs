...
use crate::gio_client::{GIOClient, GIODomain, GIOHash, GIOHint};
...
impl GIODatabase {
    ...
    async fn get_preimage(&self, hash: B256) -> Result<Vec<u8>> {
        ...
        if !response.is_ok() {
            bail!("failed to emit hint: {}", response.code)
        }
        Ok(response.response)
    }

    async fn emit_hint(&self, hint: GIOHint, input: &Vec<u8>) -> Result<()> {
